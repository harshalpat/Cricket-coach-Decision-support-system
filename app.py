...
def getvalue():
    ...
    try:
        ...
        for row in records:
            x = records2[i]
            i = i + 1
            if (x[5] == "yes" and (getchar == "t_20" or getchar == "odi")):
                st = (row[1] / 600) * 10
                hc = (row[2] / row[4]) * 10
                tr = (row[3] / y[0]) * 10
                ft = x[3]
                pitch = records10[i]
                sum = st + hc + tr + ft + int(pitch[0])
                a[0][j] = row[0]
                a[1][j] = sum
                a[2][j] = x[1]
                j = j + 1
            if (x[5] == "yes" and (getchar == "test")):
                st = (row[1] / 600) * 5
                fc = (row[2] / row[4]) * 15
                tr = (row[3] / y[0]) * 10
                ft = x[3]
                pitch = records10[i]
                sum = st + fc + tr + ft + int(pitch[0])
                a[0][j] = row[0]
                a[1][j] = sum
                a[2][j] = x[1]
                j = j + 1
        ...
        for row in records4:
            ...
            if (x[5] == "yes" and (getchar == "t_20" or getchar == "odi")):
                ...
                ft = x[3]
                sum = er + nw + db + ft + st + hc + tr + int(pitch[0])
                c[0][j] = x[1]
                c[1][j] = sum
                j = j + 1
            if (x[5] == "yes" and (getchar == "test")):
